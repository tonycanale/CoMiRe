comire.gibbs.continuous.confunder <-function(y, x, z, grid=NULL, mcmc, prior, state=NULL, seed, 
    ...
    f1i <- dnorm(y, (th1[1]+z*ga[1]) , sqrt(1/tau1[1]))
    ...
    for(ite in 2:(mcmc$nrep+mcmc$nb))
    {
      ...
      f1i <- dnorm(y, (th1[ite]+z*ga[ite]), sqrt(1/tau1[ite]))
      ...
      f1[ite,] <- dnorm(y.grid, (th1[ite]+rep(z.val,length(y.grid))*ga[ite]) , sqrt(1/tau1[ite]))
